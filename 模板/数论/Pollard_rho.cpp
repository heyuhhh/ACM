// Phollard_rho 快速质因子分解算法 
// 在 O(n^{1/3}) 的时间复杂度找所有的约数，O(n^{1/4}) 的时间复杂度内找到所有的质因子。
namespace Factor {
    const int N=1010000;
    ...
    vector<pair<ll,ll>> factorG(ll n) {
        ...
        norm();
        vector<pair<ll,ll>> d;
        for(int i=0;i<_cnt;i++) d.emplace_back(make_pair(_pr[i],_e[i]));
        return d;
    }
    bool is_primitive(ll a,ll p) {
        ...
        for(int i=0;i<sz(D);i++)
            if (powl(a,(p-1)/D[i].fi,p)==1) return 0;
        return 1;
    }
}