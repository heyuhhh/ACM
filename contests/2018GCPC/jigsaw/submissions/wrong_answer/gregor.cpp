...
int main(){
	...
	FOR(x,0,maxX) FOR(y,0,maxY) FOR(s,0,4) if (x + dx[s] < x && x + dx[s] >= 0 && y + dy[s] < y && y + dy[s] >= 0)
		if (pcs[make_pair(x,y).first][make_pair(x,y).second][s] !=
						pcs[make_pair(x+dx[s],y+dy[s]).first][make_pair(x+dx[s],y+dy[s]).second][s%2?(4-s):(2-s)])
				fail = true;
